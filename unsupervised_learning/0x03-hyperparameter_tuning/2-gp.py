...
class GaussianProcess():
    ...
    def update(self, X_new, Y_new):
        """
        Updates the kernel covariance matrix K, X data, and Y data.

        Args:
            X_new: numpy.ndarray - (1,) Represents the new sample point.
            Y_new: numpy.ndarray - (1,) Represents the new sample function
            value.
        """
        ...
        K_ = self.kernel(X_new, self.X)
        K__ = self.kernel(X_new, X_new)

        self.K = np.concatenate((self.K, K_.T), axis=1)
        K__ = np.concatenate((K_, K__), axis=1)
        self.K = np.concatenate((self.K, K__), axis=0)
